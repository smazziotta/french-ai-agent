...
A simple French conversation agent that works without local LLMs.
Uses OpenAI API (if available) or provides helpful French responses.
...
# Optional OpenAI (if user has API key)
try:
    import openai
...
MEMORY_FILE = Path.home() / ".simple_french_agent_memory.json"
...
def ask_openai(history):
    ...
    try:
        ...
        messages = [{"role": "system", "content": "Tu es un assistant IA français. Réponds toujours en français de manière naturelle et amicale."}] + history[-10:]
...
def interactive_loop():
    ...
    print("🇫🇷 Agent Français Simple - Tapez 'voice' pour la voix, 'quit' pour sortir")
    print("💡 Pour de meilleures réponses, configurez OPENAI_API_KEY dans vos variables d'environnement")
    ...
    while True:
        ...
        # Try OpenAI first, then fall back to simple responses
        response = ask_openai(memory + [{"role": "user", "content": user_text}])
